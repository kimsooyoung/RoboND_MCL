...
public:
    ...
    friend std::ostream& operator << (std::ostream& out,  Robot &robo){
        out << "[x=" + to_string(robo.x) + " y=" + to_string(robo.y) + " orient=" + to_string(robo.orient) + "]";
        return out;
    }
...
int main()
{
    ...
    auto Forward_Noise = 0.05;
    auto Turn_Noise = 0.05;
    auto Sense_Noise = 5.0;
    for (auto &particle : p)
    {
