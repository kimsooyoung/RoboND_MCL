// Compile with: g++ solution.cpp -o app -std=c++11 -I/usr/include/python2.7
// -lpython2.7
...
public:
  ...
  void set_noise(double new_forward_noise, double new_turn_noise,
                 double new_sense_noise) {
  ...
  vector<double> sense() {
    ...
    for (int i = 0; i < sizeof(landmarks) / sizeof(landmarks[0]); i++) {
      dist =
          sqrt(pow((x - landmarks[i][0]), 2) + pow((y - landmarks[i][1]), 2));
...
double evaluation(Robot r, Robot p[], int n) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    double dx = mod((p[i].x - r.x + (world_size / 2.0)), world_size) -
                (world_size / 2.0);
    double dy = mod((p[i].y - r.y + (world_size / 2.0)), world_size) -
                (world_size / 2.0);
